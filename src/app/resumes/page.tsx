...
export default function ResumeArchitectPage() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Remove this CV from your experience pool?")) return;
        try {
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-white tracking-tight">Saved Resumes (LaTeX Only)</h3>
                                    <p className="text-xs text-[#555555] font-bold uppercase tracking-widest">{resumes.filter(r => !r.isMasterTemplate).length}/4 Custom CVs Saved</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setIsAdding(true)}
                                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all border border-white/5"
                            >
                                ...
                                <form onSubmit={handleAdd} className="space-y-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex bg-[#111111] p-1 rounded-xl border border-white/5">
                                                <button
                                                    ...
                                                    className={cn("flex-1 py-2 text-xs font-bold rounded-lg", newFormat === 'latex' ? "bg-[#1d1d1d] text-white" : "text-[#444444]")}
                                                >
                                                    LaTeX
                                                </button>
                                                <button
                                                    ...
                                                    className={cn("flex-1 py-2 text-xs font-bold rounded-lg", newFormat === 'text' ? "bg-[#1d1d1d] text-white" : "text-[#444444]")}
                                                >
                                                    Raw Text
                                                </button>
                                            </div>
                                    ...
                                    <div className="flex items-center justify-end gap-3 pt-4">
                                        <button
                                            ...
                                            className="px-6 py-2.5 text-xs font-bold text-[#555555] hover:text-[#888888] transition-all"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            type="submit"
                                            className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-blue-600/20"
                                        >
                                            Record Version
                                        </button>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {isLoading ? (
                                ...
                                <div className="col-span-full py-32 flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-[40px] bg-white/[0.01]">
                                    ...
                                    <button
                                        onClick={() => setIsAdding(true)}
                                        className="bg-blue-600/10 text-blue-500 border border-blue-500/20 px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-500/20 transition-all"
                                    >
                                ...
                                resumes.map((resume: any) => (
                                    <ResumeCard
                                        ...
                                        onEdit={(res) => {
                                            ...
                                            setIsAdding(true);
                                            handleDelete(res.id); // Simple edit flow: delete and replace
                                        }}
                                    />
                                ))
                            )}
